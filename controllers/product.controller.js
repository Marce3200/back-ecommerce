...
const findOne = async (req, res) => {
  try {
    ...
    products.id=id;// para que le entrgue la info al front
    return res.json({ products });
  } catch (error) {
    return res.status(500).json({
      msg: "internal server error",
...
const update = async (req, res) => {
  ...
  try {
    ...
    ); //finByIdAndUpdate, cuando se obtiene la respuesta, por defecto devuelve el valor del ID antes de ser actualizado.El after, da el valor despues de ser actualizado.
